...
function BusinessListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface[]>(
    ...
    () =>
      getBusinesses({
        relations: ['user', 'cashier.count', 'inventory_manager.count', 'store_manager.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('cashier', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>cashier</Th>}
                  {hasAccess('inventory_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>inventory_manager</Th>
                  )}
                  {hasAccess('store_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>store_manager</Th>
                  )}
                  <Th>Actions</Th>
                </Tr>
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('cashier', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.cashier}</Td>
                    )}
                    {hasAccess('inventory_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.inventory_manager}</Td>
                    )}
                    {hasAccess('store_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.store_manager}</Td>
                    )}
                    <Td>
